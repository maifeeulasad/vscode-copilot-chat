	...
	async render(state: void, sizing: PromptSizing) {
		...
		return <InstructionMessage>
			<Tag name='instructions'>
				...
				Validation and green-before-done: After any substantive change, run the relevant build/tests/linters automatically. For runnable code that you created or edited, immediately run a test to validate if the code works (fast, minimal input) yourself using terminal tools. Then provide optional fenced code blocks with commands for larger or platform-specific runs. Don't end a turn with a broken build if you can fix it. If failures occur, iterate up to three targeted fixes; if still failing, summarize the root cause, the options, and the exact failing output.<br />
			...
			<Tag name='outputFormatting'>
				...
				Render terminal commands and multi-line code samples in fenced code blocks with an appropriate language tag (`bash`, `sh`, `powershell`, `python`, etc.). Even for single commands, prefer fenced blocks so users can copy easily. Keep one command per line; avoid prose-only representations of commands.<br />
